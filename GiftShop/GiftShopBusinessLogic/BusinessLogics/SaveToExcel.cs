	...
	static class SaveToExcel
	{
		public static void CreateDoc(ExcelInfo info)
		{
			using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
			{
				...
				uint rowIndex = 2;
				foreach (var pc in info.GiftMaterials)
				{
					InsertCellInWorksheet(new ExcelCellParameters
					{
						Worksheet = worksheetPart.Worksheet,
					...
					rowIndex++;
					foreach (var material in pc.Materials)
					{
						InsertCellInWorksheet(new ExcelCellParameters
						{
							Worksheet = worksheetPart.Worksheet,
		...
		public static void CreateStoragesDoc(StorageExcelInfo info)
		{
			using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
			{
				...
				uint rowIndex = 2;
				foreach (ReportStorageMaterialsViewModel sc in info.StorageMaterials)
				{
					InsertCellInWorksheet(new ExcelCellParameters
					{
						Worksheet = worksheetPart.Worksheet,
					...
					rowIndex++;
					foreach (var material in sc.Materials)
					{
						InsertCellInWorksheet(new ExcelCellParameters
						{
							Worksheet = worksheetPart.Worksheet,
